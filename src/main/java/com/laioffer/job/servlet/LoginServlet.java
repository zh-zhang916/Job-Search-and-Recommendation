...
public class LoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (connection.verifyLogin(body.userId, body.password)) {
            HttpSession session = request.getSession(); // get or create session: 跟当前request绑定，session可以设定时间
            session.setAttribute("user_id", body.userId); // userID是session一个attributes，没有sessionID就deny。
